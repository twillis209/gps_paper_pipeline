...
sum_stats_dat <- fread('resources/pid_ukbb_sum_stats/merged_pid_ukbb_sum_stats.tsv.gz', sep = '\t', header = T)

pruned_rsid_dat <- fread('resources/pid_ukbb_sum_stats/plink_subsets/pruned_ranges/all.prune.in', sep = ' ', header = F)

names(pruned_rsid_dat) <- 'ID'

bim_dat <- fread('resources/pid_ukbb_sum_stats/plink_subsets/all.bim', sep = '\t', header = F, col.names = c('chr', 'ID', 'Cm', 'bp', 'A1', 'A2'))
...
fwrite(sum_stats_dat, file = 'resources/pid_ukbb_sum_stats/pruned_merged_sum_stats.tsv', sep = '\t')
