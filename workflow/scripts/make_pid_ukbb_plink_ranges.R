...
for(i in 1:22) {
  ...
  fwrite(bim_dat, file = sprintf("resources/pid_ukbb_sum_stats/plink_ranges/chr%d.txt", i), row.names = F, sep = ' ', col.names = F, quote = F)
}
...
fwrite(bim_dat, file = "resources/pid_ukbb_sum_stats/plink_ranges/chrX.txt", row.names = F, sep = ' ', col.names = F, quote = F)
