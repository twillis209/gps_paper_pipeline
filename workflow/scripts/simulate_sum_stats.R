...
parser$add_argument('-a', '--hap_file', type = 'character', help = 'Path to haplotype file', default = 'resources/simgwas/1kGP/chr21_afr.hap.gz')
parser$add_argument('-l', '--leg_file', type = 'character', help = 'Path to legend file', default = 'resources/simgwas/1kGP/1000GP_Phase3_chr21.legend.gz')
...
# Leave out metadata columns 1-6
hap_dat <- hap_dat[, 7:ncol(hap_dat)]
...
mask <- leg_dat$AFR %between% c(0.01, 0.99) & apply(freq_dat, 2, var) > 0
...
if(is.null(args$causal_variant_ind)) {
  args$causal_variant_ind <- sample(1:nrow(chosen_snp_dat[AFR %between% c(0.2, 0.8)]), size = args$no_causal_variants)
}
...
if(args$no_rep == 1 ) {
  res_dat <- data.table(chosen_snp_dat[, .(id, position, a0, a1, TYPE, AFR)], zexp, zsim, vbetasim, betasim)
} else {
  res_dat <- data.table(chosen_snp_dat[, .(id, position, a0, a1, TYPE, AFR)], zexp, t(zsim), t(vbetasim), t(betasim))
}

res_dat[args$causal_variant_ind, chosen_or := args$odds_ratios]

names(res_dat) <- c('id', 'position', 'a0', 'a1', 'TYPE', 'AFR', 'zexp',
