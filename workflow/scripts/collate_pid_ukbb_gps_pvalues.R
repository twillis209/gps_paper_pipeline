...
results_files <- list.files('results/pid_ukbb', pattern = '*_gps_pvalue.tsv', full.names = T)

trait_pairs <- gsub("results/pid_ukbb/(.*)_gps_pvalue.tsv", "\\1", results_files)
...
trait_name_daf <- read.table('resources/pid_ukbb_sum_stats/traits_codes_abbrv_cases.csv', sep = ',', header = T)
