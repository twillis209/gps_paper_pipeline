...
rule munge_sum_stats:
    ...
    output:
        ...
        "results/ldsc/munged_sum_stats/chr{ch}/{effect_size}/blocks_{first}_{last}.tsv.sumstats.gz"
    params:
        output_filename = "results/ldsc/munged_sum_stats/chr{ch}/{effect_size}/blocks_{first}_{last}.tsv"
    conda:
        "envs/ldsc.yaml"
    shell:
...
rule estimate_h2:
    input:
        ["resources/ldsc/eur_w_ld_chr/%d.l2.ldscore.gz" % i for i in range(1,23)],
        ["resources/ldsc/eur_w_ld_chr/%d.l2.M_5_50" % i for i in range(1,23)],
        sum_stats = "results/ldsc/munged_sum_stats/chr{ch}/{effect_size}/blocks_{first}_{last}.tsv.sumstats.gz"
    output:
        sum_stats = "results/ldsc/h2/chr{ch}/{effect_size}/blocks_{first}_{last}.log"
    params:
        log_file_par = "results/ldsc/h2/chr{ch}/{effect_size}/blocks_{first}_{last}",
