...
rule munge_sum_stats:
    ...
    output:
        # NB: Awkward output filename due to the way the LDSC script works
         temp("results/ldsc/munged_sum_stats/chr{ch}/{ncases}_{ncontrols}/blocks_{first}_{last}_{effect_block}_{tag}.tsv.sumstats.gz")
    params:
         output_filename = "results/ldsc/munged_sum_stats/chr{ch}/{ncases}_{ncontrols}/blocks_{first}_{last}_{effect_block}_{tag}.tsv",
         signed_sumstats_col = lambda wildcards: "betasim.1,12" if wildcards.tag == 1 else "betasim.2,13",
         pvalue_col = lambda wildcards: "p.1" if wildcards.tag == 1 else "p.2"
    conda:
        "envs/ldsc.yaml"
    shell:
        """
...
rule estimate_h2:
    input:
        ["resources/ldsc/eur_w_ld_chr/%d.l2.ldscore.gz" % i for i in range(1,23)],
        ["resources/ldsc/eur_w_ld_chr/%d.l2.M_5_50" % i for i in range(1,23)],
        sum_stats = "results/ldsc/munged_sum_stats/chr{ch}/{ncases}_{ncontrols}/blocks_{first}_{last}_{effect_block}_1.tsv.sumstats.gz"
    output:
        "results/ldsc/h2/chr{ch}/{ncases}_{ncontrols}/blocks_{first}_{last}_{effect_block}.log"
    params:
        log_file_par = "results/ldsc/h2/chr{ch}/{ncases}_{ncontrols}/blocks_{first}_{last}_{effect_block}",
...
rule estimate_rg:
    input:
        ["resources/ldsc/eur_w_ld_chr/%d.l2.ldscore.gz" % i for i in range(1,23)],
        ["resources/ldsc/eur_w_ld_chr/%d.l2.M_5_50" % i for i in range(1,23)],
        sum_stats_A = "results/ldsc/munged_sum_stats/chr{ch_A}/{ncases_A}_{ncontrols_A}/blocks_{first_A}_{last_A}_{effect_block_A}_1.tsv.sumstats.gz",
        sum_stats_B = "results/ldsc/munged_sum_stats/chr{ch_B}/{ncases_B}_{ncontrols_B}/blocks_{first_B}_{last_B}_{effect_block_B}_2.tsv.sumstats.gz",
    output:
