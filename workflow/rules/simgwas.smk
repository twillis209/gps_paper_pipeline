rule run_simgwas:
    ...
    resources:
        time = "10:00:00"
    shell:
        "Rscript workflow/scripts/simulate_sum_stats.R --hap_file {input.haplotype_file} --leg_file {input.legend_file} --bim_file {input.bim_file} --no_snps {wildcards.no_snps} --no_causal_variants {wildcards.no_cv} --odds_ratios 1.3 --no_controls 10000 --no_cases 10000 --no_reps 1 -o {output} -nt {threads}"
