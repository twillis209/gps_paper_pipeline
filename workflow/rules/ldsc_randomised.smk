...
rule calculate_theoretical_rg_for_randomised_sum_stats:
    ...
    run:
        ...
        shell("Rscript workflow/scripts/ldsc/calculate_theoretical_rg_randomised_blocks.R --cv_file {input.combined_causal_variants_file} --a_blocks_file {output.a_chrom_blocks_file} --b_blocks_file {output.b_chrom_blocks_file} --odds_ratio_a {params.odds_ratio_a} --odds_ratio_b {params.odds_ratio_b} --P_a {params.sample_prevalence_A} --P_b {params.sample_prevalence_B} --K_a {params.population_prevalence_A} --K_b {params.population_prevalence_B} -o {output.theo_rg_file} -nt {threads}")
