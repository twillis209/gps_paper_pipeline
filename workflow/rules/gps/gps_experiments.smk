...
rule compute_mean_stat_for_trait_pair:
    ...
    group: "gps"
    shell:
        "workflow/scripts/gps_cpp/build/apps/computeGpsCLI -i {input} -a P.A -b P.B -c {wildcards.effect_blocks_A} -d {wildcards.effect_blocks_B} -f {wildcards.ecdf} -n {threads} -p {wildcards.pert} -e {params.epsilon_multiple} -o {output} -s mean"
