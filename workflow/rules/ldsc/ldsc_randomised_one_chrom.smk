...
def get_no_of_snps_in_chrom(wildcards):
    pass

rule munge_chrom_randomised_sum_stats:
    input:
    ...
    output:
        temp("results/simgwas/simulated_sum_stats/munged_sum_stats/{no_reps}_reps/randomised/{chr}/{ncases_A,\d+}_{ncontrols_A,\d+}_{ncases_B,\d+}_{ncontrols_B,\d+}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/seed_{seed,\d+}_{pair_label,[AB]}_{tag,\d+}_of_{tag_A,\d+}-{tag_B,\d+}.tsv.sumstats.gz")
    params:
        output_filename = "results/simgwas/simulated_sum_stats/munged_sum_stats/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/seed_{seed}_{pair_label}_{tag}_of_{tag_A}-{tag_B}.tsv",
...
rule estimate_rg_for_chrom_randomised_sum_stats:
    input:
        l2_ldscore = lambda wildcards: f"resources/ldsc/eur_w_ld_chr/{wildcards.chr.replace('chr', '')}.l2.ldscore.gz",
        l2_M5_50 = lambda wildcards: f"resources/ldsc/eur_w_ld_chr/{wildcards.chr.replace('chr', '')}.l2.M_5_50",
        sum_stats_A = "results/simgwas/simulated_sum_stats/munged_sum_stats/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/seed_{seed}_A_{tag_A}_of_{tag_A}-{tag_B}.tsv.sumstats.gz",
        sum_stats_B = "results/simgwas/simulated_sum_stats/munged_sum_stats/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/seed_{seed}_B_{tag_B}_of_{tag_A}-{tag_B}.tsv.sumstats.gz"
    output:
        "results/ldsc/simgwas/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/rg/{h2_intercept,fixed|free}_h2_{rg_intercept,fixed|free}_rg_intercept/seed_{seed}_tags_{tag_A}-{tag_B}.log"
    log:
        log = "results/ldsc/simgwas/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/rg/{h2_intercept,fixed|free}_h2_{rg_intercept,fixed|free}_rg_intercept/seed_{seed}_tags_{tag_A}-{tag_B}.actual_log"
    params:
...
rule write_out_randomised_blocks_for_chrom_for_pair:
    params:
        # TODO fix with per chrom
        no_of_blocks_in_genome = block_daf.shape[0],
        file_regex = "results/simgwas/simulated_sum_stats/block_sum_stats/(?P<no_reps>\d+)_reps/(?P<effect>\w+)(/(?P<cv>\d+)_cv)?/(?P<ncases>\d+)_(?P<ncontrols>\d+)/chr(?P<chr>\d+)/block_(?P<block_no>\d+)_seed_(?P<seed>\d+)_sum_stats\.tsv\.gz"
    output:
        a_block_file = "results/ldsc/simgwas/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/theoretical_rg/block_files/seed_{seed,\d+}_{tag_A,\d+}_{tag_A}-{tag_B,\d+}.tsv",
...
rule calculate_theoretical_rg_for_chrom_randomised_sum_stats:
    input:
        combined_causal_variants_file = "results/simgwas/combined_causal_variants.tsv",
        a_block_file = "results/ldsc/simgwas/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/theoretical_rg/block_files/seed_{seed}_{tag_A}_{tag_A}-{tag_B}.tsv",
        b_block_file = "results/ldsc/simgwas/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/theoretical_rg/block_files/seed_{seed}_{tag_B}_{tag_A}-{tag_B}.tsv"
    output:
        theo_rg_file = "results/ldsc/simgwas/{no_reps}_reps/randomised/{chr}/{ncases_A}_{ncontrols_A}_{ncases_B}_{ncontrols_B}/{effect_blocks_A}_{effect_blocks_B}_{shared_effect_blocks}/theoretical_rg/seed_{seed}_{tag_A}-{tag_B}_theo_rg.tsv",
    params:
