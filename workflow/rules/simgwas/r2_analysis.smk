rule run_r2_sweep:
    input:
        lambda wildcards: list(chain(*[get_test_files("results/simgwas/simulation_parameters.tsv", reps = 400, filetype = y, subset = f"a_blocks == \'{wildcards.blocks}\' & shared_blocks == \'{wildcards.shared_blocks}\' & ncases_A == {wildcards.ncases}", r2 = x) for x in [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35] for y in ['gps', 'li_gps']]))
    output:
    ...
    run:
        ...
        for x in [0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35]:
