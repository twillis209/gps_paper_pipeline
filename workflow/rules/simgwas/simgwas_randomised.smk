...
rule combine_randomised_block_sum_stats_for_pair:
    ...
    run:
        with open(log.log, 'w') as logfile:
            ...
            beta_column_index_A = 8 + (wildcards.no_reps * 2) + int(wildcards.tag_A)
            p_column_index_A = 8 + (wildcards.no_reps * 3) + int(wildcards.tag_A)

            ncases_column_index = 8  +  (wildcards.no_reps  *  4) + 2
            ncontrols_column_index = 8 + (wildcards.no_reps * 4) + 3
            chr_column_index = 8 + (wildcards.no_reps * 4) + 4
            block_effect_column_index = 8 + (wildcards.no_reps * 4) + 5
            ...
            beta_column_index_B = 8 + (wildcards.no_reps * 2) + int(wildcards.tag_B)
            p_column_index_B = 8 + (wildcards.no_reps * 3) + int(wildcards.tag_B)
