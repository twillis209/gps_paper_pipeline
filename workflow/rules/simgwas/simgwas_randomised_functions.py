...
def get_randomised_block_files_for_pair(wildcards):
    ...
    for k in shared_chrom_block_dict.keys():
        ...
        for v in shared_chrom_block_dict[k]:
            ...
            a_file = f"results/simgwas/simulated_sum_stats/block_sum_stats/400_reps/{k}/{wildcards.ncases_A}_{wildcards.ncontrols_A}/chr{v[0]}/block_{v[1]}_seed_{seed}_sum_stats.tsv"
            b_file = f"results/simgwas/simulated_sum_stats/block_sum_stats/400_reps/{k}/{wildcards.ncases_B}_{wildcards.ncontrols_B}/chr{v[0]}/block_{v[1]}_seed_{seed}_sum_stats.tsv"
    ...
    for chrom in range(1,23):
        for block in block_daf.query('chr == @chrom')['block']:
            ...
            if (chrom, block) not in a_chrom_block_nos and (chrom, block) not in shared_chrom_block_nos:
                a_file = f"results/simgwas/simulated_sum_stats/block_sum_stats/400_reps/null/{wildcards.ncases_A}_{wildcards.ncontrols_A}/chr{chrom}/block_{block}_seed_{seed}_sum_stats.tsv"
                block_files.append(a_file)
                a_block_files.append(a_file)

            if (chrom, block) not in b_chrom_block_nos and (chrom, block) not in shared_chrom_block_nos:
                b_file = f"results/simgwas/simulated_sum_stats/block_sum_stats/400_reps/null/{wildcards.ncases_B}_{wildcards.ncontrols_B}/chr{chrom}/block_{block}_seed_{seed}_sum_stats.tsv"
                b_block_files.append(b_file)
